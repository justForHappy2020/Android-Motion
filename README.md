...
2. `layer-list`标签实现边框阴影和背景渐变：[Android 使用xml实现边框阴影，背景渐变效果（附有RGB颜色查询对照表）](https://www.cnblogs.com/zhujiabin/p/9375893.html) **不建议使用xml实现阴影效果，推荐使用控件的elevation属性**

##### 使用矢量图

图标使用svg格式，导入方法为在drawable文件夹里new ->Vector Asset
...
### 视频缓存与管理CourseCacheUtil

由于没有找到合适的第三方库，我们编写自己的视频缓存管理工具，类名为`CourseCacheUtil`。



#### 功能介绍

通用型组件，用于处理传入的Coruse对象和Action对象列表并返回带有有效本地视频地址的，可被视频播放组件解析的Course对象；可查询已经缓存到本地的课程；

**处理Course使用实例：**

```java
//新建CourseCacheUtil对象，传入Context和CahceDir
CourseCacheUtil ccu = new CourseCacheUtil(context,getCacheDir());

//设置状态监听器，当任务处理完成(成功或失败)后触发
ccu.setOnChangeListener(new OnProcessStateChangeListener() {
	...
	if(isSuccess){//处理成功，返回的courseWithActions中有缓存好的ActionList
...
//开始处理,course为从json中解析出来的Course对象；
//actionList为包含课程动作Action对象的List，其中的每一个Action都需要 action.setOwnerCourse(course);
...
| 方法名                                  | 参数          | 返回类型      | 功能                                                         |
| --------------------------------------- | ------------- | ------------- | ------------------------------------------------------------ |
| **getAllCachedCourseList()**            | void          | List\<Course> | 查询所有已经缓存本地的课程并返回课程对象列表                 |
| **findCachedCourseById(Long courseId)** | Long courseId | Course        | 通过课程id获得已缓存的本地的课程对象                         |
| **isActionExistLocal(Long actionId)**   | Long actionId | boolean       | 根据传入的actionId查找是否有缓存到本地的有效视频文件，并返回真值 |
|                                         |               |               |                                                              |
|                                         |               |               |                                                              |
